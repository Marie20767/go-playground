...
import (
	...
	"github.com/Marie20767/go-playground/algorithms/cache"
	"github.com/stretchr/testify/assert"
)

func TestLRUCache(t *testing.T) {
	t.Run("items can be added to and retrieved from the cache", func(t *testing.T) {
		newCache, _ := cache.NewLRUCache(3)

		err := newCache.Add("name", "Snowbell")
		assert.NoError(t, err)

		value, err := newCache.Get("name")
		assert.NoError(t, err)
	...
	t.Run("items can be removed from the cache", func(t *testing.T) {
		...
		err := newCache.Add("1", "lone")
		assert.NoError(t, err)

		value, err := newCache.Get("1")
	...
	t.Run("if a key already exists in the cache, another item with the same key cannot be added", func(t *testing.T) {
		...
		err := newCache.Add("7892", "bacon")
		assert.NoError(t, err)
		err = newCache.Add("7892", "tomato")
		assert.ErrorIs(t, err, cache.ErrCacheEntryAlreadyExists)
	})
	...
	t.Run("the least recently used/read item is removed from the cache when it is full", func(t *testing.T) {
		...
		err := newCache.Add("1", "one")
		assert.NoError(t, err)
		err = newCache.Add("2", "two")
		assert.NoError(t, err)
		err = newCache.Add("3", "three")
		assert.NoError(t, err)

		assert.Equal(t, newCache.Keys(), []string{"1", "2", "3"})

		// Simulate accessing stuff from the cache in a certain order
		_, err = newCache.Get("2") // "2" is the first item we read so it will end up being the least recently used
		assert.NoError(t, err)
		_, err = newCache.Get("1")
		assert.NoError(t, err)
		_, err = newCache.Get("3")
		assert.NoError(t, err)

		// Assert order of keys is now in order of usage recency
		assert.Equal(t, []string{"2", "1", "3"}, newCache.Keys())

		// Add a new item to the cache
		err = newCache.Add("4", "four")
		assert.NoError(t, err)

		// Assert least recently used item was removed, and 4 added to end of the list
		assert.Equal(t, []string{"1", "3", "4"}, newCache.Keys())

		// Assert that the 'Least Recently Used/Read' item is no longer in the cache
		_, err = newCache.Get("2")
		assert.ErrorIs(t, err, cache.ErrCacheMiss)
	})
}
